...
def bertify_example(example):
    ...
    print(question.numpy().shape)
    print(reference.numpy().shape)
    print(candidate.numpy().shape)
    print(input_ids.numpy().shape)
    print()

    return pad(input_ids.numpy())
...
def main():
    ...
    for category in categories:
        ...
        for i, entity_id in enumerate(entity_ids[start_index:]):
            if i % 1000 == 0:
                logger.info(f"{i}/{len(entity_ids)}")
                print(f"{category_dir}/qas_concat_fit{pattern}_{i}.json)")
                with open(f"{category_dir}/qas_concat_fit{pattern}_{i}.json", 'w') as f:
            ...
            for i, qa in enumerate(qas[entity_id]):
                ...
                if not bertify_example(example):
                    print(f"remove unfit example")
                    unfit_indices.append(i)

            for k in unfit_indices[::-1]:
                qas[entity_id].pop(k)
            if len(qas[entity_id]) == 0:
                qas.pop(entity_id)

        print(f"{category_dir}/qas_concat_fit{pattern}_{start_index}.json)")
        with open(f"{category_dir}/qas_concat_fit{pattern}_{start_index}.json", 'w') as f:
            json.dump(qas, f, indent=2)
