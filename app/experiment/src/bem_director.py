...
def main():
    ...
    for category in categories:
        ...
        for i, entity_id in enumerate(entity_ids[start_index:start_index+5000]):
            if i % 100 == 0:
                logger.info(f"i/len(entity_ids): {i}/{len(entity_ids)}")
                with open(f"{category_dir}/qas_bem_{start_index+i}.json", 'w') as f:
        ...
        with open(f"{category_dir}/qas_{start_index}_bem.json", 'w') as f:
            json.dump(qas, f, indent=2)
