...
class TFC3x(keras.layers.Layer):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5, w=None):
        ...
        self.cv2 = TFConv(c1, c_, 1, 1, w=w.cv2)
        self.cv3 = TFConv(2 * c_, c2, 1, 1, w=w.cv3)
        self.m = keras.Sequential([TFCrossConv(c_, c_, k=3, s=1, g=g, e=1.0, shortcut=shortcut, w=w.m[j]) for j in range(n)])
...
def parse_model(d, ch, model, imgsz):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in [nn.Conv2d, Conv, DWConv, DWConvTranspose2d, Bottleneck, SPP, SPPF, MixConv2d, Focus, CrossConv, BottleneckCSP, C3, C3x]:
            c1, c2 = ch[f], args[0]
...
def run(
    weights=ROOT / "yolov5s.pt", imgsz=(640, 640), batch_size=1, dynamic=False,  # weights path  # inference size h,w  # batch size  # dynamic batch size
):
    # PyTorch model
