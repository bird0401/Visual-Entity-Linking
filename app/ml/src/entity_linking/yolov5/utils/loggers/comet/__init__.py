...
class CometLogger:
    ...
    def __init__(self, opt, hyp, run_id=None, job_type="Training", **experiment_kwargs) -> None:
        ...
        if run_id is None:
            ...
            self.log_asset_data(
                self.opt.hyp, name="hyperparameters.json", metadata={"type": "hyp-config-file"},
            )
            self.log_asset(
    ...
    def _get_experiment(self, mode, experiment_id=None):
        if mode == "offline":
            ...
            try:
                ...
                logger.warning("COMET WARNING: " "Comet credentials have not been set. " "Comet will default to offline logging. " "Please set your credentials to enable online logging.")
    ...
    def log_model(self, path, opt, epoch, fitness_score, best_model=False):
        ...
        for model_path in model_files:
            ...
            self.experiment.log_model(
                self.model_name, file_or_folder=model_path, file_name=name, metadata=model_metadata, overwrite=True,
            )
    ...
    def log_predictions(self, image, labelsn, path, shape, predn):
        ...
        for cls, *xyxy in filtered_labels.tolist():
            metadata.append(
                {"label": f"{self.class_names[int(cls)]}-gt", "score": 100, "box": {"x": xyxy[0], "y": xyxy[1], "x2": xyxy[2], "y2": xyxy[3]},}
            )
        for *xyxy, conf, cls in filtered_detections.tolist():
            metadata.append(
                {"label": f"{self.class_names[int(cls)]}", "score": conf * 100, "box": {"x": xyxy[0], "y": xyxy[1], "x2": xyxy[2], "y2": xyxy[3]},}
            )
    ...
    def on_train_end(self, files, save_dir, last, best, epoch, results):
        ...
        if not self.opt.evolve:
            ...
            if self.save_model:
                self.experiment.log_model(
                    self.model_name, file_or_folder=model_path, file_name=name, overwrite=True,
                )
            ...
            if self.num_classes > 1:
                for i, c in enumerate(ap_class):
                    ...
                    self.experiment.log_metrics(
                        {"mAP@.5": ap50[i], "mAP@.5:.95": ap[i], "precision": p[i], "recall": r[i], "f1": f1[i], "true_positives": tp[i], "false_positives": fp[i], "support": nt[c]}, prefix=class_name
                    )
