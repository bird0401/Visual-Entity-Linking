...
class ClearmlLogger:
    ...
    def __init__(self, opt, hyp):
        ...
        if self.clearml:
            ...
            self.task.set_base_docker("ultralytics/yolov5:latest", docker_arguments='--ipc=host -e="CLEARML_AGENT_SKIP_PYTHON_ENV_INSTALL=1"', docker_setup_bash_script="pip install clearml")
    ...
    def log_debug_samples(self, files, title="Debug Samples"):
        ...
        for f in files:
            if f.exists():
                ...
                self.task.get_logger().report_image(title=title, series=f.name.replace(it.group(), ""), local_path=str(f), iteration=iteration)
    ...
    def log_image_with_boxes(self, image_path, boxes, class_names, image, conf_threshold=0.25):
        ...
        if len(self.current_epoch_logged_images) < self.max_imgs_to_log_per_epoch and self.current_epoch >= 0:
            ...
            if self.current_epoch % self.bbox_interval == 0 and image_path not in self.current_epoch_logged_images:
                ...
                self.task.get_logger().report_image(title="Bounding Boxes", series=image_path.name, iteration=self.current_epoch, image=annotated_image)
